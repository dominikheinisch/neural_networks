...
class BasicPerceptron(Neuron):
    ...
    def is_output_correct(self, input_pair):
        err = self.learning_data[input_pair][1] - self.activation_func(input_pair)
        if err == 0:
            return True
        else:
    ...
    def activation_func(self, input_pair):
        return 1 if self.weights @ self.learning_data[input_pair][0] > 0 else self.invalid_output
